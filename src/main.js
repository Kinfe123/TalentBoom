...
function applyColorsFromSlug() {
  ...
  if (slug) {
    ...
    const defaultSlug = primaryColorValue ? `${encodeURIComponent(primaryColorValue.replace('#', ''))}-${encodeURIComponent(secondaryColorValue.replace('#', ''))}-${encodeURIComponent(primbuttnColorValue.replace('#', ''))}-${encodeURIComponent(secbuttnColorValue.replace('#', ''))}-${encodeURIComponent(accentColorValue.replace('#', ''))}` : "";
...
function updateColors() {
  const primaryColorValue = primaryColor ? primaryColor.value.replace(/#|%23/g, '') : "";
  const secondaryColorValue = secondaryColor ?  secondaryColor.value.replace(/#|%23/g, '') : "";
